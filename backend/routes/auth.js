  ...
  async (req, res) => {
    ...
    try {
      ...
      if (organizer) {
        return res.status(400).json({
          success,
          error: "Sorry, a organizer with this email already exists...",
        });
      }
...
router.get("/getusers", fetchuser, async (req, res) => {
  try {
    ...
    console.log("Filtered users:", organizers);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        ...
        return res
          .status(400)
          .json({ success, error: "Email does not exits!" });
      }
...
router.post("/otp-verify", async (req, res) => {
  ...
  res
    .status(200)
    .json({ success, message: "Password Changed Successfully..." });
});
